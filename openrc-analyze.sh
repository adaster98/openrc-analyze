...
if [ $EUID -ne 0 ]; then
	echo -e "$(basename $(readlink -nf $0)): must be superuser.";
	exit;
fi
...
get_firmware_time() {
	...
	if [ "$bootloader_launch_ns" -gt 0 ]; then
		# Convert nanoseconds to seconds
		 duration_s=$(echo "scale=3; $bootloader_launch_ns / 1000000000" | bc -l 2>/dev/null)
	fi
...
get_bootloader_time() {
	...
	if [ "$bootloader_launch_ns" -gt 0 ] && [ "$exitbootservice_end_ns" -gt 0 ]; then
		# Convert nanoseconds to seconds
		duration_s=$(echo "scale=3; ($exitbootservice_end_ns - $bootloader_launch_ns) / 1000000000" | bc -l 2>/dev/null)
	fi
...
get_kernel_time() {
	...
	local early_userspace_start=0

	# First, check for initramfs start (this is the primary kernel end marker)
	early_userspace_start=$(dmesg | grep -i "Run /init as init process" | head -1 | awk '{print $2}' | tr -d '[]' 2>/dev/null)
...
get_initramfs_time() {
	...
	local initramfs_start=0
	local duration_s=0

	initramfs_start=$(dmesg | grep -i "Run /init as init process" | head -1 | awk '{print $2}' | tr -d '[]' 2>/dev/null)
	...
	if [ -n "$initramfs_start" ] && [ -n "$initramfs_end" ]; then
		duration_s=$(echo "scale=3; $initramfs_end - $initramfs_start" | bc -l 2>/dev/null)
	fi
...
get_userspace_time_fallback() {
	...
	local userspace_start=0
	local userspace_end=0
	local duration_s=0

	userspace_start=$(dmesg | grep -i "Running init: /\|dracut: Switching root\|Run /sbin/" | head -1 | awk '{print $2}' | tr -d '[]' 2>/dev/null)

	# Use elogind starting as the marker for userspace completion
	userspace_end=$(dmesg | grep -i "elogind-daemon" | head -1 | awk '{print $2}' | tr -d '[]' 2>/dev/null)

	if [ -n "$userspace_end" ] && [ "$userspace_end" != "0.000000" ]; then
		duration_s=$(echo "$userspace_end - $userspace_start" | bc -l 2>/dev/null)
	fi
...
bootloader_time=$(get_bootloader_time)
kernel_time=$(get_kernel_time)
initramfs_time=$(get_initramfs_time)
...
if [ ! $firmware_time = "0" ]; then
    printf "\u2022 %s: %.3fs\n" "$firmware_label" "$firmware_time"
else
    printf "\u2022 $firmware_label: Enable kernel option CONFIG_ACPI_FPDT\n"
fi

if [ ! $bootloader_time = "0" ]; then
    printf "\u2022 %s: %.3fs\n" "$bootloader_label" "$bootloader_time"
else
    printf "\u2022 $bootloader_label: Enable kernel option CONFIG_ACPI_FPDT\n"
fi
